...
function useMarketplaceWrite() {
    ...
    const { isLoading: isWaiting, isSuccess, error: waitError, data: receipt } = useWaitForTransactionReceipt({
        hash,
    });
...
export function useAddService() {
    ...
    const addService = async (title: string, description: string, priceInEth: string) => {
...
export function useRequestService() {
    ...
    const requestService = async (provider: `0x${string}`, serviceIndex: bigint, clientNote: string, priceInEth: string) => {
        return mutation.writeContractAsync({
            address: CONTRACT_ADDRESS,
            abi: ABI,
...
export function useCompleteRequest() {
    ...
    const completeRequest = async (requestId: `0x${string}`, proofCid: string) => {
        return mutation.writeContractAsync({
            address: CONTRACT_ADDRESS,
            abi: ABI,
